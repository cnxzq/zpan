...
// 从环境变量或命令行参数读取配置
const PORT = process.env.PORT || (portArg ? parseInt(portArg) : 8090);
const HOST = process.env.HOST || '127.0.0.1';
const STATIC_ROOT = process.env.STATIC_ROOT || (dirArg || process.cwd());
...
const upload = multer({ 
  storage: storage,
  limits: { fileSize: 10 * 1024 * 1024 * 1024 } // 限制 10GB
});
...
app.use((req, res, next) => {
  // 只有当请求目录才需要注入
  let fullPath = path.join(STATIC_ROOT, req.path);
  try {
